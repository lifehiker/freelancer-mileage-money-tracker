...
- `[x]` Docker startup now uses `npx --no-install prisma db push --skip-generate` instead of an internal Prisma module path.
...
- `[x]` Standalone server startup path was re-verified locally by copying the standalone output, static assets, public assets, Prisma schema, and runtime `node_modules`, then running `npx --no-install prisma db push --skip-generate` plus `node server.js`.
